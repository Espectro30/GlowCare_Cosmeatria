...
- Modificación del serializador de autenticación para exponer campos phone y io al iniciar sesión.
- Actualización de los Modelos RewardProduct y Coupon añadiendo compatibilidad de imágenes locales (ImageField), soporte ampliado de dominios Unsplash (plus.unsplash.com) y campos lógicos (is_unlimited, overtime_justification).
- Ajustes en la lógica de validación de usos de cupones, ignorando el conteo de límite si el cupón posee estado infinito.