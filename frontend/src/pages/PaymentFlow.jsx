...
import { ChevronRight, ChevronLeft, Copy, CheckCircle2, ShieldCheck, Phone, Hash, Tag } from 'lucide-react';
...
export default function PaymentFlow() {
  ...
  // Tasa de cambio BCV
  const [bcvRate, setBcvRate] = useState(46.8); // Fallback
  ...
  const discountPercent = couponData ? parseFloat(couponData.discount_percentage) : 0;
  const finalMontoNum = Math.max(0, montoNum - (montoNum * (discountPercent / 100)));
  
  const montoVES = (finalMontoNum * bcvRate).toLocaleString('es-VE', { minimumFractionDigits: 2 });
  ...
  const handleValidateCoupon = async () => {
    setCouponError('');
    try {
      const data = await servicesApi.validateCoupon(couponCode);
      setCouponData(data);
    } catch (err) {
  ...
  if (step === 'methods') {
    return (
      <Wrapper bookingData={bookingData} setStep={setStep} title="Via de Pago" hideBack>
        ...
        <div className="bg-brand-800/50 border border-brand-700 rounded-3xl p-6 mb-8 relative">
          <div className="flex justify-between items-center mb-4">
            <div>
              <p className="text-brand-400 font-black text-[10px] uppercase tracking-widest mb-1">Total de la sesion</p>
              <div className="flex items-end gap-3">
                <p className="text-3xl font-black text-white">${finalMontoNum.toFixed(2)}</p>
                {couponData && <p className="text-brand-400 font-bold line-through mb-1">${montoNum.toFixed(2)}</p>}
              </div>
              <p className="text-brand-500 font-bold text-sm mt-1">
                 Bs. {montoVES} {loadingRate ? '(Cargando Tasa...)' : `(Tasa BCV: ${bcvRate.toFixed(2)})`}
              </p>
            </div>
          ...
          {/* Input de Cupon */}
          <div className="border-t border-brand-700 pt-4 flex items-center gap-2">
            <Tag className="w-5 h-5 text-brand-400" />
            <input 
              type="text" 
              placeholder="Código de Descuento" 
              className="bg-transparent border-none outline-none text-white font-bold text-sm w-full placeholder:text-brand-600 uppercase"
              value={couponCode}
              onChange={(e) => setCouponCode(e.target.value.toUpperCase())}
            />
            <button onClick={handleValidateCoupon} className="bg-brand-600 text-white font-black text-[10px] px-3 py-2 rounded-xl uppercase hover:bg-brand-500">
              Aplicar
            </button>
          </div>
          {couponError && <p className="text-red-400 text-xs mt-2 font-bold">{couponError}</p>}
          {couponData && <p className="text-green-400 text-xs mt-2 font-bold">¡Cupón de {couponData.discount_percentage}% aplicado!</p>}
        </div>
