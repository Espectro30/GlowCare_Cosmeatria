...
export default function Profile() {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const response = await apiClient.patch('/usuarios/me/', {
        first_name: form.name,
        phone: form.phone,
        bio: form.bio,
      });
      const current = JSON.parse(localStorage.getItem('glowcare_user') || '{}');
      const updated = { ...current, name: form.name };
      ...
      const current = JSON.parse(localStorage.getItem('glowcare_user') || '{}');
      const updated = { ...current, name: form.name, phone: form.phone, bio: form.bio };
      localStorage.setItem('glowcare_user', JSON.stringify(updated));
      if (avatarPreview) setAvatar(avatarPreview);
      setSaveOk(true);
      setTimeout(() => { setSaveOk(false); setEditing(false); }, 1800);
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-nut-50 p-6 rounded-3xl border border-nut-100">
                    <div className="flex items-center gap-3 text-nut-600 mb-2">
                      ...
                      <span className="font-black text-xs uppercase tracking-wider">Telefono</span>
                    </div>
                    {editing ? (
                      <input
                        type="text"
                ...
                <div className="bg-nut-50 p-6 rounded-3xl border border-nut-100">
                  <div className="flex items-center gap-3 text-nut-600 mb-3">
                    ...
                    <span className="font-black text-xs uppercase tracking-wider">Historial o Detalles</span>
                  </div>
                  {editing ? (
                    <textarea
                      value={form.bio}
                    ...
                    <p className="text-nut-700 font-medium leading-relaxed">{form.bio || 'Sin descripcion registrada.'}</p>
                  )}
                </div>
