...
import { Sparkles, UserPlus, ChevronRight, ChevronLeft, ClipboardList, AlertTriangle, Stethoscope, Eye, EyeOff } from 'lucide-react'
...
export default function Register({ isSecretaria = false }) {
  const navigate = useNavigate();

  /* ── PASO 1: Datos de cuenta ── */
  const [step, setStep] = useState(1);
  const [formData, setFormData] = useState({
    nombres: '', apellidos: '', email: '', cedula: '', telefono: '', password: '', gender: ''
  });
  ...
  const passwordValid = formData.password.length >= 8 && formData.password.length <= 12 && hasLetter && hasSpecial;
  const paso1Valido = formData.nombres && formData.apellidos && formData.email && formData.cedula && formData.telefono && passwordValid && formData.gender;
  ...
  const handleFinalRegister = async (e) => {
    ...
    try {
      ...
      await authApi.register({
        ...
        full_name: `${formData.nombres} ${formData.apellidos}`.trim(),
      ...
      if (isSecretaria) {
        alert('Paciente registrado exitosamente. Puedes registrar a otro paciente.');
        setFormData({ nombres: '', apellidos: '', email: '', cedula: '', telefono: '', password: '', gender: '' });
      ...
      <div className="min-h-screen bg-[#f0f4f0] flex items-center justify-center p-4 font-sans">
        <div className="bg-white p-8 sm:p-10 rounded-[3rem] shadow-2xl border border-nut-100 w-full max-w-md animate-in fade-in zoom-in-95 duration-500 my-8">
          ...
          <form className="space-y-4" onSubmit={handleStep1}>
            <Field label="Nombres" type="text" value={formData.nombres} onChange={v => setFormData({ ...formData, nombres: v.replace(/[0-9]/g, '').slice(0, 30) })} placeholder="Ana Maria" required maxLength={30} showCounter={true} />
            <Field label="Apellidos" type="text" value={formData.apellidos} onChange={v => setFormData({ ...formData, apellidos: v.replace(/[0-9]/g, '').slice(0, 30) })} placeholder="Gomez" required maxLength={30} showCounter={true} />
            <div>
              <Field label="Cedula de Identidad" type="text" inputMode="numeric" value={formData.cedula} onChange={v => setFormData({ ...formData, cedula: v.replace(/[^0-9]/g, '').slice(0, 8) })} placeholder="12345678" required />
              {formData.cedula.length >= 8 && <p className="text-[10px] text-red-500 font-bold mt-1 text-right">Límite alcanzado</p>}
            </div>
            <div>
              <Field label="Telefono Personal" type="text" inputMode="numeric" value={formData.telefono} onChange={v => setFormData({ ...formData, telefono: v.replace(/[^0-9]/g, '').slice(0, 12) })} placeholder="04121234567" required />
              {formData.telefono.length >= 12 && <p className="text-[10px] text-red-500 font-bold mt-1 text-right">Límite alcanzado</p>}
            </div>
            ...
            <div>
              <Field label="Contraseña" type="password" value={formData.password} onChange={v => setFormData({ ...formData, password: v.slice(0, 12) })} placeholder="••••••••" required />
              {formData.password.length > 0 && !passwordValid && (
                <p className="text-[10px] text-red-500 font-bold mt-1 text-right">
                  Mín. 8 caracteres, 1 letra y 1 carácter especial
                </p>
              )}
            </div>

            <div className="space-y-2 pt-2">
              <label className="block text-[11px] font-extrabold text-brand-700 mb-1.5 uppercase tracking-wider">Género</label>
              <div className="flex gap-3">
                {['M', 'F', 'O'].map(g => (
                  <label key={g} className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-2xl border-2 cursor-pointer transition-all text-xs font-black ${formData.gender === g ? 'border-brand-600 bg-brand-50 text-brand-800' : 'border-nut-200 bg-white text-nut-600 hover:border-brand-300'}`}>
                    <input type="radio" className="hidden" checked={formData.gender === g} onChange={() => setFormData({...formData, gender: g})} />
        ...
        <form onSubmit={handleFinalRegister} className="space-y-6">
          ...
          <SectionCard icon={<AlertTriangle className="w-5 h-5" />} title="Alergias">
            ...
            {clinicalData.alergias && (
              <div className="mt-3">
                ...
                />
                <p className={`text-right text-[11px] font-bold mt-1 ${clinicalData.alergias_detalle.length >= 200 ? 'text-red-500' : 'text-brand-400'}`}>
                  {clinicalData.alergias_detalle.length}/200
                </p>
              </div>
            )}
          </SectionCard>
          ...
          <SectionCard icon={<Stethoscope className="w-5 h-5" />} title="Condiciones Cutaneas">
            <div className="flex flex-wrap gap-2 mb-3">
              {['Ninguna', 'Rosácea', 'Psoriasis', 'Dermatitis', 'Acné crónico', 'Otro'].map(cond => {
                const isSelected = clinicalData.enfermedades_piel.includes(cond);
                return (
                  <button
                    key={cond}
                    type="button"
                    onClick={() => {
                      if (cond === 'Ninguna') {
                        setClinicalData({ ...clinicalData, enfermedades_piel: 'Ninguna', enfermedades_piel_otro: '' });
                      } else {
                        let current = clinicalData.enfermedades_piel.split(', ').filter(c => c && c !== 'Ninguna');
                        if (current.includes(cond)) {
                          current = current.filter(c => c !== cond);
                        } else {
                          current.push(cond);
                        }
                        setClinicalData({ ...clinicalData, enfermedades_piel: current.join(', ') });
                      }
                    }}
                    className={`px-3 py-1.5 rounded-full text-xs font-black border-2 transition-all ${isSelected ? 'border-brand-600 bg-brand-50 text-brand-800' : 'border-brand-100 bg-white text-brand-500 hover:border-brand-300'}`}
                  >
                    {cond}
                  </button>
                )
            ...
            {clinicalData.enfermedades_piel.includes('Otro') && (
              <textarea
                rows={2}
                maxLength={50}
                value={clinicalData.enfermedades_piel_otro}
                onChange={e => setClinicalData({ ...clinicalData, enfermedades_piel_otro: e.target.value })}
                className="w-full bg-brand-50 border border-brand-200 rounded-2xl px-4 py-3 font-bold text-brand-900 outline-none focus:border-brand-400 text-sm resize-none mt-2"
                placeholder="Especifica que otra condición tienes..."
              />
            )}
          </SectionCard>

          {/* MEDICAMENTOS */}
          <SectionCard icon={<ClipboardList className="w-5 h-5" />} title="Medicamentos Dermatológicos Actuales">
            <input
              type="text"
              maxLength={50}
            ...
            />
            <p className={`text-right text-[11px] font-bold mt-1 ${clinicalData.medicamentos_actuales.length >= 50 ? 'text-red-500' : 'text-brand-400'}`}>
              {clinicalData.medicamentos_actuales.length}/50
            </p>
          </SectionCard>
          ...
          <div>
            ...
            />
            <p className={`text-right text-[11px] font-bold mt-1 ${clinicalData.observaciones.length >= 100 ? 'text-red-500' : 'text-brand-400'}`}>
              {clinicalData.observaciones.length}/100
            </p>
          </div>
...
function Field({ label, type, value, onChange, placeholder, required, inputMode, maxLength, showCounter }) {
    ...
    <div>
      ...
      <div className="relative">
        <input
          ...
          className={`w-full px-5 py-3.5 bg-brand-50 border border-brand-200 rounded-2xl outline-none focus:ring-4 focus:ring-brand-500/20 font-bold transition-all text-brand-900 ${isPassword ? 'pr-12' : ''}`}
        ...
        {isPassword && (
          <button
            ...
            className="absolute right-4 top-1/2 -translate-y-1/2 text-brand-400 hover:text-brand-600 focus:outline-none"
          >
      ...
      {showCounter && maxLength && (
        <p className={`text-right text-[11px] font-bold mt-1 ${value.length >= maxLength ? 'text-red-500' : 'text-brand-400'}`}>
