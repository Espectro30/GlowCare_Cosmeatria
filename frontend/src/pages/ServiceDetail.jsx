...
import { Calendar, Clock, Sparkles, Info, CheckCircle2, ShieldCheck, ChevronLeft, Star, AlertCircle } from 'lucide-react';
...
export default function ServiceDetail() {
  ...
  const [hasCompletedService, setHasCompletedService] = useState(false);
    ...
    async function loadData() {
      try {
        ...
        const found = allServices.find(
          s => s.id === id || s.slug === id || s.title.toLowerCase().normalize('NFD').replace(/\p{Diacritic}/gu, '').replace(/\s+/g, '-') === id
        );
        ...
        if (found) {
          ...
          try {
             const revs = await servicesApi.getReviews(found.id);
             setReviews(revs);
          } catch(e) {}
          ...
          if (user) {
             try {
                ...
                const hasCompleted = apps.some(a => a.service === found.id && a.status.toLowerCase() === 'finalizada');
                setHasCompletedService(hasCompleted);
             } catch(e) {}
          }
        } else {
  ...
  const handleReviewSubmit = async (e) => {
    ...
    setReviewSuccess('');
    try {
      const res = await servicesApi.createReview(service.id, reviewForm);
      setReviews([...reviews, res]);
      setReviewForm({ rating: 5, comment: '' });
      setReviewSuccess('¡Reseña publicada con éxito!');
    } catch (err) {
      setReviewError(err.response?.data?.detail || 'Error al publicar reseña. Debes haber completado el servicio.');
    }
  };
          ...
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="lg:w-5/12 relative h-[400px] lg:h-auto overflow-hidden min-h-[400px]">
              ...
              <div className="absolute bottom-10 left-10 text-white z-10">
                ...
                <div className="flex items-center gap-2 mt-3">
                  ...
                  <span className="text-brand-300 text-xs font-bold ml-1">— Protocolo Clinico Certificado ({reviews.length} reseñas)</span>
                </div>
              </div>
            </div>
            ...
            <div className="lg:w-7/12 p-10 lg:p-16">
              ...
              <div className="mt-16 border-t border-brand-100 pt-10">
                 <h3 className="font-black text-brand-950 text-2xl mb-6">Opiniones de Pacientes</h3>
                 
                 {user && user.role === 'cliente' && hasCompletedService && (
                    <form onSubmit={handleReviewSubmit} className="mb-10 bg-brand-50 p-6 rounded-3xl border border-brand-100">
                       <p className="text-[10px] font-black text-brand-500 uppercase tracking-widest mb-3">Deja tu opinión</p>
                       {reviewError && <p className="text-red-500 text-xs font-bold mb-3">{reviewError}</p>}
                       {reviewSuccess && <p className="text-green-600 text-xs font-bold mb-3">{reviewSuccess}</p>}
                       <div className="flex items-center gap-2 mb-4">
                          {[1,2,3,4,5].map(num => (
                             <button type="button" key={num} onClick={() => setReviewForm({...reviewForm, rating: num})}>
                                <Star className={`w-6 h-6 transition-all ${num <= reviewForm.rating ? 'text-yellow-400 fill-current' : 'text-brand-200'}`} />
                             </button>
                          ))}
                       </div>
                       <textarea required rows="2" className="w-full bg-white p-4 rounded-2xl border border-brand-200 outline-none focus:border-brand-500 font-bold text-brand-900 resize-none text-sm mb-3" placeholder="¿Cómo fue tu experiencia?" value={reviewForm.comment} onChange={e=>setReviewForm({...reviewForm, comment: e.target.value})}></textarea>
                       <button type="submit" className="bg-brand-900 text-white font-black text-[10px] uppercase tracking-widest px-6 py-3 rounded-xl hover:bg-brand-700 transition-all">Publicar Reseña</button>
                    </form>
                 )}
                 
                 <div className="space-y-4">
                    {reviews.length === 0 ? (
                       <p className="text-brand-400 font-bold text-sm italic">Aún no hay opiniones para este servicio. ¡Sé el primero!</p>
                    ) : reviews.map(rev => (
                       <div key={rev.id} className="bg-white border border-brand-100 p-5 rounded-2xl">
                          <div className="flex justify-between items-start mb-2">
                             <div>
                                <p className="font-black text-brand-900 text-sm">{rev.user_name}</p>
                                <p className="text-[10px] text-brand-400 font-bold">{new Date(rev.created_at).toLocaleDateString()}</p>
                             </div>
                             <div className="flex">
                                {[1,2,3,4,5].map(num => (
                                   <Star key={num} className={`w-3 h-3 ${num <= rev.rating ? 'text-yellow-400 fill-current' : 'text-brand-100'}`} />
                                ))}
                             </div>
                          </div>
                          <p className="text-brand-700 text-sm font-medium leading-relaxed">{rev.comment}</p>
                       </div>
                    ))}
                 </div>
              </div>
            ...
            <div className="space-y-4 mb-10 max-h-[40vh] overflow-y-auto pr-2">
              {schedules.length === 0 ? (
                <div className="bg-red-50 p-6 rounded-3xl border border-red-200 text-center">
                  <p className="text-red-600 font-bold">No hay horarios disponibles por el momento para este servicio.</p>
                </div>
              ) : schedules.map((sched) => {
                const uniqueId = sched.virtual_id || sched.id;
                return (
                  <label
                    key={uniqueId}
                    className={`flex items-center justify-between p-6 rounded-3xl border-4 cursor-pointer transition-all ${selectedSchedule === uniqueId ? 'border-brand-600 bg-brand-50' : 'border-nut-100 bg-white hover:border-brand-200'}`}
                  >
                    <div className="flex items-center gap-5">
                      <input
                        type="radio"
                        className="w-5 h-5 text-brand-600 cursor-pointer"
                        checked={selectedSchedule === uniqueId}
                        onChange={() => setSelectedSchedule(uniqueId)}
                      />
                    ...
                    {selectedSchedule === uniqueId && <CheckCircle2 className="w-6 h-6 text-brand-500" />}
                  </label>
                );
              })}
            </div>
